...
static void check_osiris_library(void) {
    ...
    if (osiris) {
        ...
        // TODO: Install hooks here once we verify function signatures
        log_message("Hooks NOT installed (safe mode - verifying stability first)");

        // Don't close - we need this handle for hooks later
        // dlclose(osiris);
    } else {
