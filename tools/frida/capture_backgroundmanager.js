/**
 * Frida script to capture BackgroundManager pointer during character creation.
 *
 * Run with: frida -p <PID> -l capture_backgroundmanager.js
 *
 * Trigger: Navigate to background selection in character creation.
 *
 * NOTE: Background has NO FixedString Name field - only TranslatedString DisplayName.
 * The Name field offset is 0 in the config, so names won't be extracted.
 */

const MANAGER_COUNT_OFFSET = 0x7C;
const MANAGER_ARRAY_OFFSET = 0x80;
const BACKGROUND_SIZE = 0x80;  // 128 bytes per background (estimate)
const OUTPUT_FILE = "/tmp/bg3se_backgroundmanager.txt";
...
function validateAndCapture(funcName, ptr, argIndex) {
    ...
    try {
        ...
        // Backgrounds: expect 10-30 entries
        if (count >= 5 && count < 100 && !array.isNull()) {
            console.log("\n[+] " + funcName + " - Found BackgroundManager candidate at arg[" + argIndex + "]: " + ptr);
            ...
            console.log("[+] Wrote BackgroundManager to " + OUTPUT_FILE);
            console.log("[+] In BG3SE: Ext.StaticData.LoadFridaCapture('Background')");
            console.log("[!] NOTE: Background entries have NO Name field - only GUID available");
        }
    } catch (e) {
        // Invalid pointer
    }
...
// TypeContext traversal for BackgroundManager
var mStatePtr = mainModule.base.add(0x083c4a68);

function findViaTypeContext() {
    if (capturedManager) return;
    console.log("\n[*] Scanning TypeContext for BackgroundManager...");
    ...
    try {
        ...
        while (!typeInfo.isNull() && count < 200) {
            try {
                ...
                if (typeName.indexOf("BackgroundManager") !== -1) {
                    console.log("[+] Found BackgroundManager in TypeContext: " + mgrPtr);
...
console.log("\n[*] BackgroundManager capture script loaded");
console.log("[*] Navigate to background selection in character creation");
