/**
 * Frida script to capture OriginManager pointer during character creation.
 *
 * Run with: frida -p <PID> -l capture_originmanager.js
 *
 * Trigger: Navigate to origin selection in character creation.
 */

const MANAGER_COUNT_OFFSET = 0x7C;
const MANAGER_ARRAY_OFFSET = 0x80;
const ORIGIN_SIZE = 0x180;  // 384 bytes per origin (estimate)
const OUTPUT_FILE = "/tmp/bg3se_originmanager.txt";
...
function validateAndCapture(funcName, ptr, argIndex) {
    ...
    try {
        ...
        // Origins: expect 7-20 entries (7 base origins + custom)
        if (count >= 5 && count < 50 && !array.isNull()) {
            console.log("\n[+] " + funcName + " - Found OriginManager candidate at arg[" + argIndex + "]: " + ptr);
            ...
            console.log("[+] Wrote OriginManager to " + OUTPUT_FILE);
            console.log("[+] In BG3SE: Ext.StaticData.LoadFridaCapture('Origin')");
        }
    } catch (e) {
        // Invalid pointer
    }
...
// TypeContext traversal for OriginManager
var mStatePtr = mainModule.base.add(0x083c4a68);

function findViaTypeContext() {
    if (capturedManager) return;
    console.log("\n[*] Scanning TypeContext for OriginManager...");
    ...
    try {
        ...
        while (!typeInfo.isNull() && count < 200) {
            try {
                ...
                if (typeName.indexOf("OriginManager") !== -1) {
                    console.log("[+] Found OriginManager in TypeContext: " + mgrPtr);
...
console.log("\n[*] OriginManager capture script loaded");
console.log("[*] Navigate to origin selection in character creation");
